...
public class DadosDemonstracaoSeeder implements CommandLineRunner {
    ...
    private void criarLotacaoEAlertas() {
        ...
        AlertaOperacional gpsAlerta = new AlertaOperacional(
            ...
            "CRITICO",
            "PENDENTE",
            "Perda de sinal de telemetria da Viatura #106 (" + viaturas.get(viaturas.size() - 1).getMatricula() + ") na Linha 2. Nenhum pacote GPS recebido há 15 minutos.",
            "GPS Telemetria",
            "Viatura ID: 106, Matrícula: " + viaturas.get(viaturas.size() - 1).getMatricula() + ", Última Telemetria: " + LocalDateTime.now().minusMinutes(15).toString().replace("T", " ").substring(0, 16) + " no Hospital de Braga"
        );
