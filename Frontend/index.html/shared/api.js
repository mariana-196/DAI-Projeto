...
document.addEventListener("DOMContentLoaded", () => {
    // Role-based UI visibility & security checks
    const userRole = sessionStorage.getItem("user_role");
    const currentPath = window.location.pathname;

    // Hide Auditoria from menu if not admin
    const navAuditoria = document.getElementById("nav-auditoria");
    if (navAuditoria && userRole !== "ADMINISTRADOR") {
        navAuditoria.style.display = "none";
    }

    // Redirect to dashboard if trying to access auditoria directly without being ADMIN
    if (currentPath.endsWith("auditoria.html") && userRole !== "ADMINISTRADOR") {
